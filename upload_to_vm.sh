...
echo -e "  3. Install deps: ${BLUE}pip install -r requirements_cuda.txt${NC}"
echo -e "  4. Split data: ${BLUE}python3 split_judge_data.py datasets/judge_eval/judge_training_data_merged_1512.json${NC}"
echo -e "  5. Train (with early stopping): ${BLUE}python3 pt_app/eval_model/judge_train_cuda.py --epochs 10 --batch_size 2 --base_adapter adapters/${BEST_JUDGE}${NC}"
echo -e ""
echo -e "${BLUE}Key changes from previous training:${NC}"
echo -e "  • Using --base_adapter to continue from BEST_JUDGE checkpoint"
echo -e "  • Removed --skip_eval to enable validation & early stopping"
echo -e "  • Early stopping will save best checkpoint (likely 2-4 epochs)"
echo -e "  • Training on 1121 examples (low-mid scores 1-8)"
echo ""
